...
```CREATE TABLE `sosmed`.`messages` (`id` INT NOT NULL AUTO_INCREMENT, `senderid` INT NOT NULL, `receiverid` INT NOT NULL, `desc` VARCHAR(255) NOT NULL, `img` VARCHAR(255) NULL, `createdat` DATETIME NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `id_UNIQUE` (`id` ASC), CONSTRAINT `sender_id` FOREIGN KEY (`senderid`) REFERENCES `sosmed`.`users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT `receiver_id` FOREIGN KEY (`receiverid`) REFERENCES `sosmed`.`users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE); ```
